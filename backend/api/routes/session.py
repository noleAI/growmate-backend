...
async def create_session(
    request: SessionCreateRequest, user: dict = Depends(get_current_user)
):
...
async def interact(
    session_id: str, request: InteractionRequest, user: dict = Depends(get_current_user)
):
    # Standard interact loop
    updated_beliefs = bayesian_tracker.update_beliefs(
        request.action_type, request.response_data or {}
    )
